        ...
        if line.startswith("- ") or line.startswith("* "):
            if not in_list:
                out.append(
                    ...
                    "padding-left:24px;color:#334155;line-height:1.6;'>"
                )
                in_list = True
            ...
            if line.startswith("###"):
                ...
                out.append(
                    f"<h3 style='color:#0F172A;margin-top:24px;"
                    f"margin-bottom:10px;font-size:16px;"
                    f"border-bottom:1px solid #E2E8F0;"
                    f"padding-bottom:6px;'>{h}</h3>"
                )
            elif line.startswith("##"):
                h = line.replace("#", "").strip()
                out.append(
                    f"<h2 style='color:#0F172A;margin-top:28px;"
                    f"margin-bottom:12px;font-size:18px;"
                    f"border-bottom:2px solid #CBD5E1;"
                    f"padding-bottom:6px;'>{h}</h2>"
                )
            elif line:
                out.append(
                    f"<p style='margin-top:0;margin-bottom:14px;"
                    f"color:#334155;line-height:1.6;'>{line}</p>"
                )
...
def metric_tile(label, val, color="#1E293B"):
    return f"""
    <td width="33.3%" style="padding:16px 12px; background-color:#FFFFFF;
        ...
        <div style="font-size:22px; color:{color}; font-weight:800;
...
def alm_indicator(label, val, color="#1E293B"):
    return f"""
    <td width="25%" style="padding:12px 10px;
        border-right:1px solid #E2E8F0; text-align:center;">
        <div style="font-size:11px; color:#64748B; font-weight:600;
            text-transform:uppercase; letter-spacing:0.5px;">{label}</div>
        <div style="font-size:15px; color:{color}; font-weight:700;
...
def build_market_dashboard(market):
    ...
    def aval_bps(key):
        # FRED OAS series stored as percent — multiply by 100 for bps display
    ...
    <table width="100%" cellpadding="0" cellspacing="0"
           style="margin-top:16px; background-color:#F8FAFC;
      ...
      <tr>
        ...
        <td width="25%" style="padding:12px 10px; text-align:center;">
            <div style="font-size:11px; color:#64748B; font-weight:600;
                text-transform:uppercase; letter-spacing:0.5px;">VIX</div>
            <div style="font-size:15px; color:{vix_color}; font-weight:700;
                margin-top:4px;">{aval_pct("VIX", "{:.1f}")}</div>
        </td>
      </tr>
    </table>
    """
    ...
    return f"""
    <table width="100%" cellpadding="0" cellspacing="0"
           style="margin-top:20px; border-collapse:separate; border-spacing:8px 8px;">
      <tr>
...
def build_tag_badges(tags):
    ...
    for tag in tags:
        ...
        badges += f"""
        <span style="display:inline-block; background-color:{color}15;
            color:{color}; border:1px solid {color}40; font-size:10px;
            font-weight:700; padding:2px 8px; margin-right:6px;
            margin-bottom:6px; border-radius:12px; text-transform:uppercase;
...
def build_impact_section(
    ...
    for category, articles in category_buckets.items():
        ...
        rows += f"""
        <tr>
            <td style="padding:24px 0 12px 0; font-size:18px;
        ...
        for idx, article in enumerate(matching[:config.MAX_ARTICLES_PER_SECTION]):
            tag_html = build_tag_badges(article.get("tags", ["GENERAL"]))
            bg_color = "#FFFFFF" if idx % 2 == 0 else "#F8FAFC"

            rows += f"""
            <tr>
                <td style="padding:20px 16px; background-color:{bg_color};
                    border-bottom:1px solid #F1F5F9; border-radius:6px;">
                    ...
                    <div style="color:#475569; font-size:13px;
                        ...
                        {html_escape(article.get('snippet', ''))}
                    </div>
                    <div style="color:#94A3B8; font-size:11px; margin-top:10px;
                        font-weight:600; text-transform:uppercase;
                        letter-spacing:0.5px;">
                        {html_escape(article.get('source', ''))}
                        &nbsp;&bull;&nbsp;
                        {html_escape(article.get('date', ''))}
                    </div>
                </td>
            </tr>
            """
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="margin-top:40px;">
        <tr>
            <td style="background-color:{impact_color}10; color:{impact_color};
                padding:14px 20px; font-weight:800; font-size:15px;
                border-left:4px solid {impact_color};
                border-radius:0 6px 6px 0; letter-spacing:1px;">
...
def build_email_html(market_data, category_buckets, llm_summary):
    ...
    <body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',
        Roboto,Helvetica,Arial,sans-serif; margin:0; padding:30px 10px;
        background-color:#F1F5F9; color:#0F172A;">

        <table width="100%" cellpadding="0" cellspacing="0"
               style="max-width:760px; margin:0 auto; background-color:#FFFFFF;
                      border-radius:12px;
            ...
            <tr>
                <td style="background-color:#0F172A; padding:32px 40px;
                    text-align:left; border-bottom:4px solid #3B82F6;">
                    <h1 style="color:#F8FAFC; margin:0; font-size:26px;
                        font-weight:800; letter-spacing:-0.5px;">
                        Life &amp; Annuity Actuarial Intelligence
                    </h1>
                    <p style="color:#94A3B8; margin:8px 0 0 0; font-size:14px;
                        font-weight:500; letter-spacing:0.5px;">
                        ARC | Springline &nbsp;&bull;&nbsp; {today}
                    </p>
                </td>
            </tr>
            ...
            <tr>
                <td style="background-color:#F8FAFC;
                    ...
                    <strong>Actuarial Resources Corporation</strong>
                    &nbsp;&bull;&nbsp; Internal Consulting Briefing Pipeline<br>
                    <span style="font-size:11px; color:#94A3B8;">
                        Sources: NAIC LATF &nbsp;&bull;&nbsp;
                        SOA &nbsp;&bull;&nbsp; AAA &nbsp;&bull;&nbsp;
                        LIMRA &nbsp;&bull;&nbsp; AM Best &nbsp;&bull;&nbsp;
                        ThinkAdvisor &nbsp;&bull;&nbsp; Insurance Journal
                        &nbsp;&bull;&nbsp; Carrier Management
                        &nbsp;&bull;&nbsp; Reinsurance News
                        &nbsp;&bull;&nbsp; Milliman &nbsp;&bull;&nbsp;
                        Federal Register &nbsp;&bull;&nbsp; SEC EDGAR
                        &nbsp;&bull;&nbsp; Google News
                    </span>
                </td>
            </tr>
